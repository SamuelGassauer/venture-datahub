...
import { Check, ImageIcon, ImageOff, Loader2 } from "lucide-react";
...
export function LogoPicker({
  ...
  const handleSave = useCallback(async () => {
    ...
    const isNoImage = selected === NO_IMAGE;
    const value = isNoImage ? null : selected;
    ...
    try {
      ...
      if (res.ok) {
        const body = await res.json().catch(() => ({}));
        onSelect(isNoImage ? null : (body.value ?? selected));
        onOpenChange(false);
      } else {
        const body = await res.json().catch(() => ({}));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 py-2">
          ...
          {!loading && (
            <div className="grid grid-cols-4 gap-3">
              {/* "No image" option — always shown */}
              <button
                onClick={() => setSelected(selected === NO_IMAGE ? null : NO_IMAGE)}
              ...
              >
                ...
                <span className="mt-1.5 text-[11px] font-medium text-muted-foreground/60">Kein Bild</span>
                {selected === NO_IMAGE && (
                  <div className="absolute top-1.5 right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-400 text-white">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={!selected || saving}>
            ...
            {selected === NO_IMAGE ? "Logo entfernen" : "Save"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
