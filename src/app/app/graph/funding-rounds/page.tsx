...
import { ArrowUpDown, Search, CircleDollarSign, Trash2 } from "lucide-react";
...
export default function GraphFundingRoundsPage() {
  ...
  async function handleDelete() {
    ...
    if (!confirm(`${selected.size} Funding Round(s) unwiderruflich löschen?`)) return;

    setDeleting(true);
    try {
      const res = await fetch("/api/graph-funding-rounds/delete", {
        method: "POST",
      ...
      <div className="glass-status-bar flex items-center gap-3 px-4 py-2.5 shrink-0">
        ...
        {selected.size > 0 && (
          <button
            ...
            disabled={deleting}
            className="ml-2 flex items-center gap-1.5 rounded-full bg-red-500/8 px-3 py-1 text-[12px] font-medium text-red-500 transition-colors hover:bg-red-500/15 disabled:opacity-50"
          >
              ...
              <TableBody>
                {filtered.map((r) => (
                  <TableRow
                    ...
                    className="lg-inset-table-row text-[13px]"
                  >
                    ...
                    <TableCell
                      className="py-1.5 px-2 font-semibold text-foreground/85 cursor-pointer"
                      onClick={() => {
